...
func Scan(rows gorm.Rows, db *gorm.DB, mode gorm.ScanMode) {
	...
	if tcacheStore, ok := db.Get("oracle:cacheStore"); !ok {
		cacheStore = &sync.Map{}
		db.Set("oracle:cacheStore", cacheStore)
	} else {
		cacheStore = tcacheStore.(*sync.Map)
	}
	...
	case map[string]interface{}, *map[string]interface{}:
		if initialized || rows.Next() {
			...
			db.RowsAffected++
			_ = db.AddError(rows.Scan(values...))
	...
	default:
				...
				for idx, column := range columns {
					if field := sch.LookUpField(column); field != nil && field.Readable {
						...
						if rel, ok := sch.Relationships.Relations[names[0]]; ok {
							...
							if f := rel.FieldSchema.LookUpField(dbName); f != nil && f.Readable {
								fields[idx] = f
		...
		case reflect.Slice, reflect.Array:
			...
			BEGIN:
				...
				scanIntoStruct(db, rows, elem, values, fields, joinFields)
		...
		case reflect.Struct, reflect.Ptr:
			if initialized || rows.Next() {
				...
				scanIntoStruct(db, rows, reflectValue, values, fields, joinFields)
			}
		default:
			_ = db.AddError(rows.Scan(dest))
		}
...
func scanIntoStruct(db *gorm.DB, rows gorm.Rows, reflectValue reflect.Value, values []interface{}, fields []*schema.Field, joinFields [][]*schema.Field) {
	...
	for idx, field := range fields {
		...
		if len(joinFields) == 0 || len(joinFields[idx]) == 0 {
			_ = db.AddError(field.Set(db.Statement.Context, reflectValue, convertToLiteral(db.Statement, values[idx], reflectValue, field)))
			...
			if !isNilPtrValue { // ignore if value is nil
				...
				_ = db.AddError(f.Set(db.Statement.Context, relValue, values[idx]))
			}
		}
